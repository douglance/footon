...
export function renderHome(): Response {
  const content = `<section class="panel"><p class="prompt">$ footon share --safe</p><h1>Share the work. Keep the secrets.</h1><p class="lede">Claude and Codex threads become clean, unlisted links. Raw transcripts never leave your machine.</p><div class="actions"><a class="button" href="/install">[ install_cli ]</a><a class="button secondary" href="/connect">[ connect_agent ]</a></div><dl class="facts"><div><dt>scan</dt><dd>local + edge</dd></div><div><dt>upload</dt><dd>approved prose only</dd></div><div><dt>access</dt><dd>unlisted + revocable</dd></div></dl></section>`
