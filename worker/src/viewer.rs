...
const rail=document.querySelector('.minimap');const map=document.querySelector('.minimap ol');const markers=[...document.querySelectorAll('.map-marker')];const targets=markers.map(marker=>({marker,message:document.getElementById(marker.getAttribute('href')?.slice(1)||marker.dataset.messageId)}));const filters=[...document.querySelectorAll('[data-filter-role]')];const canvas=document.createElement('canvas');const context=canvas.getContext('2d',{alpha:true});const texture=document.createElement('canvas');const textureContext=texture.getContext('2d',{alpha:true});const reducedMotion=matchMedia('(prefers-reduced-motion: reduce)').matches;let scale=1,frame=0;
...
canvas.tabIndex=0;canvas.setAttribute('role','navigation');canvas.setAttribute('aria-label','Thread minimap');canvas.addEventListener('pointerdown',event=>scrollTo({top:event.clientY/scale-innerHeight/2,behavior:reducedMotion?'auto':'smooth'}));rail?.prepend(canvas);for(const filter of filters)filter.addEventListener('change',applyFilters);addEventListener('scroll',schedule,{passive:true});addEventListener('resize',layout);applyFilters();
";
