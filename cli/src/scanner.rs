...
// This local-only pattern pack synthesizes high-signal families used by
// Gitleaks and Kingfisher: provider prefixes, auth assignments, connection
// strings, private keys, and paths. It never validates a candidate against a
// live service, which avoids turning sanitization into credential use.
struct Rule {
    kind: &'static str,
    regex: Regex,
}
...
static RULES: LazyLock<Vec<Rule>> = LazyLock::new(|| {
    [
        ...
        ("GITHUB", r"\b(?:gh[pousr]_[A-Za-z0-9]{36}|github_pat_[A-Za-z0-9_]{70,})\b"),
        ("SLACK", r"\bxox[baprs]-[A-Za-z0-9-]{10,72}\b"),
        ("JWT", r"\beyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]+\b"),
        ("API_KEY", r"\b(?:sk-(?:proj-|ant-)?|AIza|SG\.|npm_|pypi-)[A-Za-z0-9._-]{20,}\b"),
        ("CONNECTION", r"\b(?:postgres(?:ql)?|mysql|mongodb(?:\+srv)?|redis|amqp)://[^\s]+"),
        ("ASSIGNMENT", r#"(?i)\b(?:api[_-]?key|secret|token|password|passwd|auth)[A-Za-z0-9_-]*\s*[:=]\s*['\"]?[A-Za-z0-9._~+/=-]{20,}['\"]?"#),
